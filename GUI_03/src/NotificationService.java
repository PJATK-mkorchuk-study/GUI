...
public
    ...
    public void addListener(NotificationListener listener) {
        if(listenerCount == listeners.length) {
            NotificationListener[] newArr = new NotificationListener[listeners.length * 2];
    ...
    public void addFilter(NotificationFilter filter) {
        if(filterCount == filters.length) {
            NotificationFilter[] newArr = new NotificationFilter[filters.length * 2];
            for(int i = 0; i < newArr.length; i++) {
                newArr[i] = filters[i];
            }
            filters = newArr;
        ...
        for(int i = 0; i <= filterCount - 1; i++) {
            if(!filters[i].accept(message)) {
                ...
                for(int j = 0; j <= channelCount - 1; j++) {
                    ...
                    for(int k = 0; k <= listenerCount - 1; k++) {
                        listeners[k].onFailure(channels[k].getType(), message, reason);
...
//TODO 12
    public static Result[] sort(Result[] results, ResultComparator comparator) {
        ...
        for(int i = 0; i < sorted.length - 2; i++) {
            for(int j = 0; j < sorted.length - 2 - i; j++) {
